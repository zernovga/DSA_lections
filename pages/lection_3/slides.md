...
# Алгоритмы, программирование и структуры данных<br>Лекция 3. Сортировка.
...
Пусть требуется упорядочить $N$ элементов: $R_1, R_2, …, R_n$. Каждый элемент представляет из себя запись $R_j$, содержащую некоторую информацию и ключ $K_j$, управляющий процессом сортировки. На множестве ключей определено отношение порядка $<$ так, чтобы для любых трёх значений ключей $a, b, c$ выполнялись следующие условия:
